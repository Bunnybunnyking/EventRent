...
/**
 * All URLs included in `sitemap.xml` (indexable, canonical). Keep in sync with `robots.ts` disallow rules.
 */
export function buildIndexablePaths(): string[] {
  const tentPaths = buildTentInventoryPaths();
  const staticPaths = [
    "/", "/sitemap", "/tent-rentals", "/tent-rentals/jobsite-coverage", "/tents", ...tentPaths, "/tents/gallery", "/events", "/events/birthdays", "/rsvp", "/rsvp/create", "/av-games", "/yard-games", "/bounce-houses", "/table-chair-rentals", "/wedding-tent-rentals", "/corporate-event-rentals", "/rental-inventory", "/party-packages", "/about", "/contact", "/faq", "/planning", "/planning-tools", "/quick-event-planner", "/backyard-party-checklist", "/tent-seating-reference", "/wishlist", "/party-guides", ...partyGuideSlugs.map((slug) => `/party-guides/${slug}`), "/packages/most-booked-event-setups", "/how-it-works", "/reviews-and-real-events", "/case-studies", "/service-areas", ...eventLandingSlugs.map((slug) => `/events/${slug}`), ];
  const townPaths = townList.map(townToPath);
  return [...staticPaths, ...townPaths];
}
