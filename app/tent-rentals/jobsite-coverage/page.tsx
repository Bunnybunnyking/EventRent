...
import {
  jobsiteDeliveryFeeDisplay,
  jobsiteFourteenDayRates,
  jobsiteMinimumOrderDisplay,
  jobsitePackages,
  jobsiteWeekdayPricingNote,
} from "@/lib/jobsite-coverage-data";
import { createPageMetadata } from "@/lib/metadata";
import { business } from "@/lib/site-data";

export const metadata = createPageMetadata({
  title: `Jobsite Tent Rentals & Heat Relief | Shade & Hydration Stations | ${business.primaryCity} CT`, description:
    "Contractor-focused jobsite tent rentals in Connecticut: shade stations, hydration stations, and heat relief packages for roofing, construction, paving, and landscaping crews. Weekday jobsite rates (Mon–Fri through Friday 11 AM) on extended 14-day construction tent pricing, delivery and setup.", path: "/tent-rentals/jobsite-coverage", });

const coverageGuide = [
  {
    title: "10x10 Tent", body: "Ideal for small break areas, quick shade coverage, and smaller crews.", }, {
    title: "10x20 Tent", body: "Great for roofing crews and medium-size outdoor jobs needing shaded break space.", }, {
    title: "20x20 Tent", body: "Best for larger crews, lunch areas, and extended jobsite coverage.", }, ] as const;

const addOns = [
  "Extra coolers", "Additional water and ice", "Tables and chairs", "Fans", "Misting fans", "Sidewalls", "Refill service for longer jobs", ];

const bestFor = [
  "Roofing crews", "Construction sites", "Paving crews", "Landscaping teams", "Exterior renovation crews", "Utility and maintenance crews", ];
  ...
  return (
    <>
      <ServiceSchema
        ...
        description="Temporary tent coverage, hydration stations, and heat relief packages for Connecticut contractors, roofing, construction, paving, landscaping, and outdoor crews. Weekday jobsite rates for Monday–Friday programs through Friday 11 AM. Delivery, setup, and extended 14-day jobsite rentals."
      ...
      <section className="border-b border-stone-200/80 bg-[#f7f5f1] py-8 sm:py-10">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <Breadcrumb
            className="mb-3"
            items={[
              { label: "Home", href: "/" }, { label: "Tent Rentals", href: "/tent-rentals" }, { label: "Jobsite Coverage" }, ]}
          />
          ...
          <div className="mt-10 space-y-4 text-left">
            ...
            <p className="rounded-xl border border-amber-200/80 bg-amber-50/90 px-4 py-3 text-sm font-medium text-stone-800">
              Minimum weekday jobsite rental order: <span className="font-semibold">{jobsiteMinimumOrderDisplay}</span>
            </p>
          </div>
          ...
          <div className="mt-12">
            ...
            <p className="mx-auto mt-4 max-w-3xl rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-center text-xs leading-relaxed text-stone-700 sm:text-sm">
              <strong className="font-semibold text-stone-900">Weekday jobsite window:</strong>{" "}
              {jobsiteWeekdayPricingNote}
            </p>
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {jobsitePackages.map((pkg) => (
                <article
                  key={pkg.name}
          ...
          <div className="mt-16 text-left">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-stone-600">
              These are <strong className="font-semibold text-stone-800">14-day weekday jobsite tent rental</strong> rates for roofing, construction, exterior work, crew break areas, staging, and temporary weather coverage, built for longer runs on active sites, not single-day parties. Same{" "}
              <strong className="font-semibold text-stone-800">Monday–Friday through Friday 11:00 AM</strong> window as the package pricing above.
            </p>

            <div className="mt-6 overflow-x-auto rounded-2xl border border-stone-200 bg-white shadow-sm">
              <table className="min-w-full text-left text-sm" aria-label="14-day weekday jobsite tent rental rates by size">
                <thead>
                ...
                <tbody>
                  {jobsiteFourteenDayRates.map((row) => (
            ...
            <p className="mt-4 text-sm font-medium text-stone-800">
              Additional one-time delivery fee: {jobsiteDeliveryFeeDisplay} per tent (weekday jobsite program)
            </p>
            <p className="mt-2 max-w-3xl text-xs leading-relaxed text-stone-500">
              14-day weekday jobsite pricing shown above is part of our extended-use construction rental program. Delivery, setup, site conditions, and availability may affect final quote. Outside the Monday–Friday / Friday 11:00 AM window, standard jobsite rates apply.
            </p>
          </div>
          ...
          <section className="mt-14 rounded-3xl bg-gradient-to-r from-[#1a1d20] to-[#272b30] p-8 text-stone-100 md:p-10" aria-labelledby="jobsite-cta-heading">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-stone-300 sm:text-base">
              Ask about Shade Station, Hydration Station, and Heat Relief packages, or lock in extended construction tent rentals with 14-day weekday jobsite pricing. Share your site, dates, and crew size; we will recommend coverage that fits the work.
            </p>
