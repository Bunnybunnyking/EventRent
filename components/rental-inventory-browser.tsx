...
function RentalItemCard({ item, dense = false }: { item: RentalBrowserItem; dense?: boolean }) {
  ...
  return (
    <article
      className={`group flex h-full flex-col overflow-hidden rounded-xl border border-stone-200/95 bg-white shadow-[0_1px_0_rgba(0,0,0,0.04),0_6px_20px_rgba(15,15,15,0.06)] transition hover:border-stone-300 hover:shadow-[0_10px_28px_rgba(15,15,15,0.08)] ${dense ? "" : ""}`}
    ...
    >
      <div className="relative aspect-[5/3] w-full bg-gradient-to-br from-stone-100 to-stone-200/80">
        {item.image ? (
        ...
        {item.featured ? (
          <span className="absolute left-2 top-2 rounded-full bg-[#1a1a1a]/90 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#f0e6c8]">
            Popular
          </span>
        ) : null}
      </div>
      <div className={`flex flex-1 flex-col ${dense ? "p-3" : "p-4"}`}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-stone-100 pt-3">
          ...
          {hasGs ? <span className="text-[10px] text-emerald-700">Live catalog</span> : null}
        </div>
        <div className="mt-2 flex flex-wrap gap-2">
          <button
            ...
            title="Wishlist integration coming with Goodshuffle"
          >
            Wishlist
          </button>
          <Link
            href="/contact#quote"
            className="rounded-full border border-stone-300 bg-white px-3 py-1.5 text-[11px] font-semibold text-stone-800 transition hover:bg-stone-50"
          >
            Ask on quote
          </Link>
        </div>
...
function CardGrid({ items, dense }: { items: RentalBrowserItem[]; dense?: boolean }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
        <RentalItemCard key={item.id} item={item} dense={dense} />
      ))}
    </div>
  );
}

export function RentalInventoryBrowser() {
  const allForFeatured = [...tentInventoryCards, ...chairInventoryCards, ...tableBanquetCards, ...tableRoundCards, ...tableCocktailSpecialtyCards, ...lightingCards, ...heatingCards];
  ...
  return (
    <>
      <section className="border-b border-stone-200/80 bg-[#f7f5f1] pb-6 pt-6 sm:pb-8 sm:pt-8">
        <div className={shell}>
          <Breadcrumb className="mb-4" items={[{ label: "Home", href: "/" }, { label: "Rental inventory" }]} />
          ...
          <ul className="mt-8 flex flex-wrap gap-x-6 gap-y-2 border-t border-stone-200/80 pt-6 text-sm text-stone-700">
          ...
          <nav
            className="sticky top-0 z-20 mt-8 flex flex-wrap gap-2 border-y border-stone-200/90 bg-[#f7f5f1]/95 py-3 backdrop-blur-md [-webkit-backdrop-filter:blur(8px)]"
      ...
      <section id="inv-tents" className="scroll-mt-24 border-b border-stone-200 bg-stone-50 py-10 sm:py-14">
        <div className={shell}>
          ...
          <div className="max-w-3xl">
            <h2 className="text-2xl font-semibold tracking-tight text-stone-900 sm:text-3xl">Tents &amp; tent systems</h2>
            <p className="mt-2 text-sm leading-relaxed text-stone-600 sm:text-base">
              Our strongest inventory depth—sizes from compact shade through expandable runs and marquee connectors.{" "}
              <Link href="/tent-rentals" className="font-semibold text-stone-900 underline decoration-[#b78a2d]/40 underline-offset-4">
                Tent rentals overview
              </Link>{" "}
              explains modular planning; here, browse what we typically stock.
            </p>
          </div>
          <div className="mt-8">
            <CardGrid items={tentInventoryCards} />
          </div>
        </div>
      </section>
      ...
      <section id="inv-chairs" className="scroll-mt-24 border-b border-stone-200 bg-white py-10 sm:py-14">
        <div className={shell}>
          <h2 className="text-2xl font-semibold tracking-tight text-stone-900 sm:text-3xl">Seating — chairs</h2>
          ...
          <div className="mt-8">
            <CardGrid items={chairInventoryCards} dense />
          </div>
        </div>
      </section>
      ...
      <section id="inv-tent-guide" className="scroll-mt-24 border-b border-stone-200 bg-white py-10 sm:py-12">
        <div className={shell}>
          <h2 className="text-xl font-semibold text-stone-900 sm:text-2xl">Tent types explained</h2>
          <p className="mt-2 max-w-2xl text-sm text-stone-600">Short definitions to align vocabulary before we talk site and schedule.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {tentTypesExplained.map((t) => (
              <div key={t.id} className="rounded-xl border border-stone-200 bg-stone-50/80 p-4">
                <h3 className="text-sm font-semibold text-stone-900">{t.title}</h3>
                <p className="mt-1.5 text-xs leading-snug text-stone-600">{t.blurb}</p>
              </div>
            ))}
          </div>
          ...
          <div className={shell}>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {block.items.map((item) => (
                <RentalItemCard key={item.id} item={item} dense />
              ))}
            </div>
          </div>
        </section>
