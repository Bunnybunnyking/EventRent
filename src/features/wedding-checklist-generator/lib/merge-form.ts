...
export function mergeFormState(prev: WeddingChecklistFormState, patch: WeddingChecklistFormPatch): WeddingChecklistFormState {
  return {
    ...
    basics: { ...prev.basics, ...patch.basics },
    setup: { ...prev.setup, ...patch.setup },
    venue: { ...prev.venue, ...patch.venue },
    priorities: patch.priorities ?? prev.priorities,
  };
}
...
export function normalizeLoadedForm(raw: Partial<WeddingChecklistFormState>): WeddingChecklistFormState {
  ...
  return {
    ...
    basics: { ...d.basics, ...raw.basics },
    setup: { ...d.setup, ...raw.setup },
    venue: { ...d.venue, ...raw.venue },
    priorities: raw.priorities?.length ? raw.priorities : d.priorities,
  };
}
