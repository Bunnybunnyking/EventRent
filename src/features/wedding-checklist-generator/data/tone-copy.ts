...
export function buildHeadline(ctx: ContentContext): string {
  if (ctx.mode === "quick") {
    const variants = [
      "Outdoor & tent essentials checklist",
      "Your streamlined outdoor wedding checklist",
      "Tent, weather & flow—your essentials list",
    ];
    return pickVariant(variants, `${ctx.variantSeed}-h`);
  }
  const variants = [
    "Your outdoor & tented wedding checklist",
    "A full readiness checklist for tent and lawn celebrations",
    "Timeline, site & vendors—built for outdoor weddings",
  ];
...
export function buildCopyLeads(ctx: ContentContext): WeddingChecklistCopyLeads {
  const tfForget = pickVariant(
    [
      "Easy-to-miss details that often surface a week (or a day) before the wedding:",
      "Practical reminders couples and planners double-check close to the date:",
      "Small logistics that make a big difference when you’re outdoors or under tent:",
    ],
    `${ctx.variantSeed}-tf`,
  );
  ...
  const tfEnh = pickVariant(
    [
      "Thoughtful upgrades that often improve comfort and photos—none are required:",
      "Optional upgrades that tend to pair well with your selections:",
      "Polish and comfort ideas that match outdoor and tented receptions:",
    ],
    `${ctx.variantSeed}-en`,
  );
  ...
  const tfGuest = pickVariant(
    [
      "Warm touches guests tend to remember—pick what fits your vibe:",
      "A few guest-forward ideas that work well with outdoor flow:",
      "Small experience upgrades that pair naturally with tent and lawn settings:",
    ],
    `${ctx.variantSeed}-gi`,
  );

  return {
    thingsForget: tfForget,
    enhancements: tfEnh,
    guestIdeas: tfGuest,
  };
}
